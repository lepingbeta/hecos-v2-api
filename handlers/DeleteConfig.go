...
func DeleteConfigHandler(c *gin.Context) {
	...
	if unsafe.Sizeof(form) > 0 {
		...
		if err := c.ShouldBindJSON(&form); err != nil {
			respData = types.ResponseData{
				...
				MsgKey: "config_delete_config_params_error",
				Data:   nil,
			}
		...
		if !ok {
			respData = types.ResponseData{
				...
				MsgKey: "config_delete_config_invalid_validator",
				Data:   nil,
			}
	...
	pointer := DeleteConfig.DeleteConfig{Params: form, C: c, DataM: dataM, Filter: dataM, Result: bson.M{}, FindOpts: options.Find(), FindOneOpts: options.FindOne()}

	// 对参数进行复杂验证
	pointer.DeleteConfigValidator2()
	if pointer.Err != nil {
		respData = types.ResponseData{
			Status: types.ResponseStatus.Error,
