...
import (
	...
	"tangxiaoer.shop/dahe/hecos-v2-api/services/config/ConfigList"
...
func ConfigListHandler(c *gin.Context) {
	...
	if unsafe.Sizeof(form) > 0 {
		...
		if err := c.ShouldBindQuery(&form); err != nil {
			respData = types.ResponseData{
				...
				MsgKey: "config_config_list_params_error",
				Data:   nil,
			}
		...
		if !ok {
			respData = types.ResponseData{
				...
				MsgKey: "config_config_list_invalid_validator",
				Data:   nil,
			}
	...
	pointer := ConfigList.ConfigList{Params: form, C: c, DataM: dataM, Filter: dataM, Result: bson.M{}, FindOpts: options.Find(), FindOneOpts: options.FindOne()}

	// 对参数进行复杂验证
	pointer.ConfigListValidator2()
	if pointer.Err != nil {
		respData = types.ResponseData{
			Status: types.ResponseStatus.Error,
